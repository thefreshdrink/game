...
// Правка в чате 2026-09-11: «плавнее и медленнее» — гравитация и снос
// вниз, отскоки мягче (меньше восстановления), карты запускаются реже.
const CASCADE_STAGGER = 0.08;    // запуск карт друг за другом, сек (было 0.06)
...
const CASCADE_GRAV_FRAC = 1.6;   // гравитация = h * этого (было 2.6)
const PLACE_DUR = 0.85;          // длительность финального отскока в слот, сек (было 0.62)
const PLACE_ARC_FRAC = 0.14;     // высота горба этого отскока над прямой к слоту — доля высоты
const TRAIL_MAX = 5;             // точек следа на карту — короткий, только намёк
    ...
    draw(ctx, w, h) {
      ...
      cards.forEach((c, i) => {
        ...
        if (c.cx === undefined) {
          ...
          c.vx = (home.x - c.cx) * 0.5;
          c.vy = h * (0.02 + pseudoRnd(i, 2) * 0.03);
          c.bounces = 0;
          c.placing = false;
          c.placeT = 0;
          c.trail = [];
        ...
        if (c.placing) {
          ...
          if (c.cy >= floorY) {
            ...
            if (c.bounces >= CASCADE_MAX_BOUNCES) {
              ...
              // Отскок почти вертикальный, каждый следующий слабее и мягче
              // (было 0.5/0.28 — читалось резковато для «плавнее»).
              const e = c.bounces === 1 ? 0.4 : 0.2;
