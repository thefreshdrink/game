...
// Наведённая карта — только чуть-чуть приподнимается, соседи чуть-чуть
// расступаются (правка в чате, 2026-08-19): не нужно гарантированно
// убирать нахлёст целиком — карта всё равно может быть частично закрыта
// той, что лежит спереди в естественном порядке вера, это нормально.
// Прошлые версии (100%+ высоты, потом 55%) были попыткой полностью
// убрать нахлёст и читались «слишком высоко» — заменены на лёгкий сдвиг.
const LIFT_FRAC = 0.12;
const SPREAD_PUSH = 22; // насколько расступаются соседи, на шаг индекса
const SPREAD_RATE = 14; // скорость лерпа расступания
...
export function createDeckScreen({ input, images, goto }) {
  ...
  return {
    ...
    update(dt) {
      ...
      cards.forEach((c, i) => {
        const targetX = i === hoveredIndex ? 0 : (hoveredIndex !== null ? (i - hoveredIndex) * SPREAD_PUSH : 0);
        const targetY = i === hoveredIndex ? -c.h * LIFT_FRAC : 0;
        const rate = 1 - Math.exp(-SPREAD_RATE * dt);
        c.spread.x += (targetX - c.spread.x) * rate;
