...
// Толщина плиты везде одна (PLATE_H 32).
//
// Профиль высот (правка в чате 2026-08-31: «платформы на разной высоте,
// повыше»): дельты разнятся — в основном ±32, один подъём −64 (P4, «повыше»;
// нормальный свайп берёт с запасом — пик дуги ~150–215px). НО суммарно
// дорога идёт ВНИЗ (канон §8.3, open-questions A1): финал ниже старта,
// подъёмов меньше, чем спусков. Высоты относительно старта по плитам:
// 0, −32, 0, −64, −32, 0, +32, 0, +32.
export const P1_WIDTH = 384;
export const START_WALK = 176;

const DATA = [
  { dy: 0,   w: P1_WIDTH },                     // P1 — старт, длинная
  { dy: -32, w: 160, gap: G_B, jump: true },    // лёгкий подъём
  { dy: 32,  w: 128, gap: G_A, jump: true },    // ступень вниз
  { dy: -64, w: 160, gap: G_A, jump: true },    // ПОДЪЁМ повыше — щель короткая, прощает вялый флик
  { dy: 32,  w: 128, gap: G_B, jump: true },    // вниз
  { dy: 32,  w: 128, gap: G_A, jump: true },    // ещё вниз — дорога пошла под уклон
  { dy: -32, w: 160, gap: G_B, jump: true },    // короткий подскок
  { dy: 32,  w: 128, gap: G_A, jump: true },    // вниз
  { dy: 32,  w: 224, gap: G_B, jump: true },    // финальная — ниже старта, длинная, дальше удержание
];
...
export const ARRIVE_MAIN_W = 320;  // плита прибытия (кратно 32; правка в чате 2026-09-10: длиннее — под траву/цветы/кустик/лесенку)
