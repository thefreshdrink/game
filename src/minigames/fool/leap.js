...
const ARRIVE_DUR = 1.0;   // такт 3 — проявление земли (правка в чате 2026-09-10: быстрее, «сразу потом предсказание»)
...
export function createLeapScreen({ input, images, goto }) {
  ...
  function drawFallSequence(ctx, w, h) {
    ...
    // тянуло тапать): сперва плита + трава/цветы/кустик/лесенка, потом
    // РЕЗКО солнце, дальше сразу предсказание с заголовком.
    ...
    // Жизнь на плите — сразу следом за плитой.
    drawArrivalLife(ctx, groundX, gw, groundY, clamp01(p / 0.3));
    // Солнце — резко, ближе к концу такта, вровень с приходом заголовка.
    drawArrivalSun(ctx, w * 0.78, h * 0.19, t, clamp01((p - 0.55) / 0.15));
  ...
  return {
    ...
    update(dt, w = 430, h = 932) {
      ...
      if (state === 'walk') {
        ...
        // Искры — яркие белые крупицы навстречу; ливень в момент удара
        // молнии (начало), дальше редкий ручеёк.
        const boltBurst = prog < 0.12;
        sparkT -= dt;
        if (sparkT <= 0) {
          sparkT = boltBurst ? 0.02 : 0.12 + Math.random() * 0.14;
        ...
        if (stateT >= ARRIVE_DUR + 0.15) goto('prediction');
      }
